...
public class Piece {
	...
	*/
	public Piece(TPoint[] points) {
		...
		HashMap<Integer, Integer> lowests = new HashMap<>();
		for (int i = 0; i < body.length; i++) {
			defineWidthAndHeight(i);
			defineLowestPoint(lowests, body[i].x, body[i].y);
		}

		skirt = new int[width];
		composeSkirt(lowests);
	}

	private void composeSkirt(HashMap<Integer, Integer> lowests) {
		// assign lowest y point to each x index
		for (int x: lowests.keySet()) {
			skirt[x] = lowests.get(x);
		}
	}

	private void defineLowestPoint(HashMap<Integer, Integer> lowests, int x, int y) {
		if (lowests.containsKey(x) && y < lowests.get(x)) {
			lowests.put(x, y);
			return;
		} else if (!lowests.containsKey(x)) {
			lowests.put(x, y);
		}
	}

	private void defineWidthAndHeight(int i) {
		if (body[i].x + 1 > width) {
			width = body[i].x + 1;
		}
	...
	*/
	...
	public Piece computeNextRotation() {
		...
		for (int i = 0; i < body.length; i++) {
			nextBody[i] = rotateCoordinates(body[i].x, body[i].y);
		}

		return new Piece(nextBody); // YOUR CODE HERE
	}

	// using formula ...
	private TPoint rotateCoordinates(int x, int y) {
		return new TPoint(height - 1 - y, x);
	}
	...
	*/
	public boolean equals(Object obj) {
		...
		// YOUR CODE HERE
		TPoint[] objBody = ((Piece) obj).getBody();
		for (int i = 0; i < body.length; i++) {
			if (body[i].x != objBody[i].x || body[i].y != objBody[i].y) {
				return false;
			}
		}
		return true;
	...
	*/
	private static Piece makeFastRotations(Piece root) {
		Piece first = root.computeNextRotation();
		root.next = first;

		Piece second = first.computeNextRotation();
		first.next = second;

		Piece third = second.computeNextRotation();
		second.next = third;
